...
class InfrastructureDoc:
    ...
    caches: list[CacheDoc] = field(default_factory=list)
    cache_keys: list[CacheKeyDoc] = field(default_factory=list)
    ...
    def from_infrastructure(cls, infra: Infrastructure) -> InfrastructureDoc:
        return cls(
            ...
            caches=[CacheDoc.from_cache(c) for c in infra.caches],
            cache_keys=[CacheKeyDoc.from_cache_key(k) for k in infra.cache_keys],
        )
